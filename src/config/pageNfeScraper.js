...
const scraperObject = {
    ...
    async scraper(browser, chaveAcesso){
        ...
        try{
            ...
            // Quando for Danfe precisa clicar no botão "Ver Detalhes"
            // e aguardar carregar a página com os dados
            // esse botão não existe quande é NFC-e, vai direto para os dados
            // então se não encontrar o botão, continua a execução

            const viewDetailsButtonSelector = '.btn.btn-info.btn-view-det.button-compensation-min-479';
            const element = await page.$(viewDetailsButtonSelector); // retorna null se não encontrar

            if (element) {

                // Espera o clique e a navegação ao mesmo tempo
                await Promise.all([
                    page.waitForNavigation({ waitUntil: 'networkidle0' }),
                    page.click(viewDetailsButtonSelector),
                ]);

                console.log('Clique realizado e navegação concluída!');

            } else {
                console.log(`Elemento "${viewDetailsButtonSelector}" não encontrado, clique não realizado.`);
            }
            ...
            const dataNf = await page.evaluate(async()=>{
                ...
                try {
                    ...
                    const transporte = (id) => {
                        const el = selectorAll('#Transporte span')[id];
                    ...
                    const cobranca = () => {
                        ...
                        return {
                            ...
                            integracaoPgto: removeSpace(dataValue(dataLength - 5)),
                    ...
                    console.log('dataScrapper', dataScrapper); //exibe no console do navegador
                    return dataScrapper
                } catch (error) {
                    console.log('erro scraper',error)
                }
